...
class MainWindow(QMainWindow):
    ...
    def __init__(self, project):
        ...
        self.setWindowTitle(f"LC Stencil Studio 0.7.0 - {project.name}")
    ...
    def new_project(self):
        ...
        self.setWindowTitle(f"LC Stencil Studio 0.7.0 - {self.project.name}")
        self.clear_properties()
        self.statusBar().showMessage("Nuovo progetto creato", 3000)

    def import_image(self):
        filename, _ = QFileDialog.getOpenFileName(
            self, "Importa immagine", "", "Immagini (*.png *.jpg *.jpeg *.bmp)"
        )
        if not filename:
            return
    ...
    def create_toolbar(self):
        ...
        for text, slot in [
            ("Importa", self.import_image),
            ("Centra", self.center_selected),
            ("Duplica", self.duplicate_selected),
            ("Elimina", self.delete_selected),
            ("Reset", self.reset_selected),
            ("Adatta materiale", self.fit_selected),
        ]:
            action = QAction(text, self)
            action.triggered.connect(slot)
    ...
    def create_statusbar(self):
        status = QStatusBar()
        status.showMessage("Pronto - CAD 0.7.0")
    ...
    def create_ui(self):
        ...
        left = QListWidget()
        left.addItems(["Progetti", "Livelli", "Materiali", "Preset"])
        ...
        properties = QGroupBox("Proprietà CAD")
        form = QFormLayout(properties)
        self.x_spin = self._spin()
        self.y_spin = self._spin()
        self.w_spin = self._spin(0.01, 100000.0)
        self.h_spin = self._spin(0.01, 100000.0)
        self.rotation_spin = QDoubleSpinBox()
        self.rotation_spin.setRange(-360.0, 360.0)
        self.rotation_spin.setDecimals(1)
        self.rotation_spin.setSuffix("°")
        self.aspect_lock = QCheckBox("Mantieni proporzioni")
        self.aspect_lock.setChecked(True)
        form.addRow("X", self.x_spin)
        form.addRow("Y", self.y_spin)
        form.addRow("Larghezza", self.w_spin)
        form.addRow("Altezza", self.h_spin)
        form.addRow("Rotazione", self.rotation_spin)
        form.addRow(self.aspect_lock)
        apply_btn = QPushButton("APPLICA MISURE")
        apply_btn.clicked.connect(self.apply_properties)
        form.addRow(apply_btn)
        right_layout.addWidget(properties)

        actions = QGroupBox("Azioni")
        actions_layout = QVBoxLayout(actions)
        for text, slot in [
            ("Centra sul materiale", self.center_selected),
            ("Adatta al materiale", self.fit_selected),
            ("Duplica", self.duplicate_selected),
            ("Reset trasformazioni", self.reset_selected),
            ("Elimina", self.delete_selected),
        ]:
    ...
    def clear_properties(self):
        ...
        for spin in (self.x_spin, self.y_spin, self.w_spin, self.h_spin, self.rotation_spin):
            spin.setValue(0)
        self._updating_properties = False
    ...
    def refresh_properties(self):
        ...
        if not geometry:
            ...
            return
        self._updating_properties = True
        self.x_spin.setValue(geometry["x"])
        self.y_spin.setValue(geometry["y"])
        self.w_spin.setValue(geometry["width"])
        self.h_spin.setValue(geometry["height"])
        self.rotation_spin.setValue(geometry["rotation"])
        self._updating_properties = False

    def apply_properties(self):
        if self._updating_properties:
            return
        ok = self.canvas.set_selected_geometry(
            x=self.x_spin.value(), y=self.y_spin.value(),
            width=self.w_spin.value(), height=self.h_spin.value(),
            rotation=self.rotation_spin.value(),
            lock_aspect=self.aspect_lock.isChecked(),
        )
        if ok:
            self.refresh_properties()
            self.statusBar().showMessage("Misure CAD applicate", 2000)

    def center_selected(self):
        if self.canvas.center_selected():
            self.refresh_properties()
            self.statusBar().showMessage("Selezione centrata", 2000)

    def duplicate_selected(self):
        if self.canvas.duplicate_selected():
            self.refresh_properties()
            self.statusBar().showMessage("Elemento duplicato", 2000)

    def delete_selected(self):
        if self.canvas.delete_selected():
            self.clear_properties()
            self.statusBar().showMessage("Elemento eliminato", 2000)

    def reset_selected(self):
        if self.canvas.reset_selected_transform():
            self.refresh_properties()
            self.statusBar().showMessage("Trasformazioni ripristinate", 2000)

    def fit_selected(self):
        if self.canvas.fit_selected_to_material():
            self.refresh_properties()
            self.statusBar().showMessage("Selezione adattata al materiale", 2000)
